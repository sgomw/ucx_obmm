...
- Preserve the measured geometry defaults unless the task is explicitly to retune them: `obmm_nc` keeps NC eager/control on `FIFO_SIZE=64`, `FIFO_ELEM_SIZE=64`, `BCOPY_SEG_SIZE=32768`, `BW=3400MBs`, `obmm_cc` currently uses the computed CC-local eager prefix with `FIFO_SIZE=8`, `BCOPY_SEG_SIZE=65600`, a 16-entry local desc pool, and 128 B desc prefixes, and both roles still share 2 MiB CC bulk windows. Keep FIFO and bcopy strides 64-byte aligned unless new measurements justify changing them.
