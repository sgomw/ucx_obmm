...
- **No-hardware obmm verification** (run from `ucx/` after install): `./install/bin/ucx_info -d -t obmm_nc`, `./install/bin/ucx_info -c | grep -i OBMM`, `nm -D ./install/lib/libuct.so | grep uct_obmm`
...
- `obmm_iface.c` owns transport capabilities, reachability, progress, and slot allocation. The current public TLs are `obmm_nc` (NC remote/eager), `obmm_cc` (same-node low-latency eager), and `obmm_bulk` (same-node/inter-node CC bulk bcopy).
- `obmm_ep.c` is the send path. `obmm_nc` uses NC short lanes plus paired bcopy descriptors, `obmm_cc` uses the CC short lane plus paired bcopy descriptors, and `obmm_bulk` uses NC control descriptors plus CC windows.
...
- The transport is active-message only. `obmm_nc` and `obmm_cc` both support `am_short` + `am_bcopy`, while `obmm_bulk` is bcopy-only; pending, connect-to-iface, synchronous callbacks, and inter-node reachability remain role-dependent. PUT/GET/RMA/zcopy/atomics remain unsupported unless a separate design changes that.
- Do not call `obmm_export`, `obmm_unexport`, `obmm_import`, `obmm_unimport`, `obmm_preimport`, or `obmm_unpreimport` inside the UCT transport. `obmm_set_ownership` is allowed only on the isolated `obmm_bulk` CC window path, not on the NC eager path.
...
- Preserve the measured geometry defaults unless the task is explicitly to retune them: `obmm_nc` / `obmm_bulk` use `FIFO_SIZE=64`, `FIFO_ELEM_SIZE=64`, `BCOPY_SEG_SIZE=32768`, `BW=3400MBs`, while `obmm_cc` uses a computed CC-local prefix with `BCOPY_SEG_SIZE=32768`. Keep FIFO and bcopy strides 64-byte aligned unless new measurements justify changing them.
