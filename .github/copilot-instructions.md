...
- `obmm_iface.c` owns transport capabilities, reachability, progress, and slot allocation. The current public TL is unified `obmm`; one iface owns an NC eager slot, an NC bulk-control slot, a CC eager slot, and CC bulk windows.
- `obmm_ep.c` is the send path. The unified `obmm` TL routes internally: same-node short/eager prefers the CC slot, cross-node short/eager prefers the NC slot, and large bcopy transfers use CC windows published over the NC bulk-control slot.
...
- The transport is active-message only. The unified `obmm` TL supports `am_short` + `am_bcopy`; pending, connect-to-iface, synchronous callbacks, and inter-node reachability remain enabled, while PUT/GET/RMA/zcopy/atomics stay unsupported unless a separate design changes that.
...
- Preserve the measured geometry defaults unless the task is explicitly to retune them: unified `obmm` keeps NC eager/control on `FIFO_SIZE=64`, `FIFO_ELEM_SIZE=64`, `BCOPY_SEG_SIZE=32768`, `BW=3400MBs`, keeps the computed CC-local eager prefix with `BCOPY_SEG_SIZE=32768`, and keeps 2 MiB CC bulk windows. Keep FIFO and bcopy strides 64-byte aligned unless new measurements justify changing them.
