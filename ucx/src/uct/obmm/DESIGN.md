...
## Region layout (v2 + dual-tier short fast path)
...
|   short_lane_table_hdr (tiny + regular active masks)  |
|   tiny_short_lane[64]  (64B-cell tiny-short lanes)    |
|   short_lane[64]       (248B regular-short lanes)     |
...
The slot now reserves two fixed SPSC short areas:

- tiny tier:
  - `tiny_short_lane_count = 64`
  - `tiny_short_lane_fifo_size = 8`
  - `tiny_short_lane_elem_size = 64`
- regular tier:
  - `short_lane_count = 64`
...
Both tiers use the same deterministic lane mapping. This matches the current
two-node / `slot_count=32` environment and removes per-message CAS from the
entire supported `am_short` path.
...
short-lane area =     128 + 64*(64 + 128 + 8*64) + 64*(64 + 128 + 8*256)
                    = ~ 184 KiB / slot
ctl + slot data = 128 + short-lane area + 64*(64+32768) = ~2236 KiB / slot
slot_count      =     32
total           = ~ 69.9 MiB / 128 MiB                   ✓
```
These defaults are chosen for the current latency-first split:

- the public `am_short` limit remains the regular SPSC lane budget
  (`248` total header+payload bytes)
- internally, payloads up to the tiny tier budget (`56` total bytes) use the
  64B-cell tiny lane first
- payloads above tiny budget and up to 248 bytes use the regular 256B-cell
  short lane
...
geometry error. With the current dual-tier short design there is little
...
`am_short` now has two internal tiers:

1. **tiny-short SPSC fast path**: total short bytes fit in the fixed
   `tiny_short_lane_elem_size - offsetof(header)` budget (currently 56 bytes).
2. **regular short SPSC path**: total short bytes fit in the fixed
   `short_lane_elem_size - offsetof(header)` budget (currently 248 bytes).

Both tiers use deterministic SPSC lanes and publish by advancing the lane head
— no per-message CAS and no legacy FIFO fallback.
...
2. select the tier:
      if total_bytes <= tiny_max_short: use tiny lane
      else:                             use regular short lane
3. if lane meta does not match current sender/receiver identities, refresh
   `meta.{sender_slot_index,sender_generation,sender_pid}`
   and reset `ctl.{head,tail}=0`
4. lazily mark the chosen tier's active-mask bit on the first send from this ep
5. use ep-local cached head
6. if head - cached_tail >= lane_fifo_size:
        bus_load_fence; refresh cached_tail; recheck;
        if still full: return UCS_ERR_NO_RESOURCE
7. write elem[head & (lane_fifo_size - 1)] inline
8. bus_store_fence()
9. lane->ctl.head = head + 1
10. update ep-local cached head
```

This removes the success-path remote CAS from all supported `am_short`
traffic while letting tiny messages move less NC payload than the 248B lane.
Messages larger than the regular short budget are expected to use `am_bcopy`.
...
Both short tiers copy `[header|payload]` out of the NC lane element before
invoking the callback. Receiver-side progress keeps a local tail cache per lane
...
rounds. Receiver progress checks the tiny tier first and then the regular short
tier, and each tier keeps its own hot-lane hint: it first retries the lane that
most recently produced data and only falls back to reading that tier's shared
active bitmap if the hint misses. If short-lane progress found work and there
...
| flag                            | v1 | v2 | notes                       |
|---------------------------------|----|----|-----------------------------|
| AM_SHORT                        |  ✓ |  ✓ | public max = 248; internal tiny+regular SPSC tiers |
