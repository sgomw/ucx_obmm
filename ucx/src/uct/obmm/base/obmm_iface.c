...
ucs_config_field_t uct_obmm_iface_config_table[] = {
    ...
    {"LATENCY", "80ns", "OBMM device latency fallback",
     ucs_offsetof(uct_obmm_iface_config_t, latency), UCS_CONFIG_TYPE_TIME},

    {"OVERHEAD", "100ns", "OBMM software overhead fallback",
     ucs_offsetof(uct_obmm_iface_config_t, overhead), UCS_CONFIG_TYPE_TIME},

    {"PRIORITY", "0", "OBMM transport priority",
     ucs_offsetof(uct_obmm_iface_config_t, priority), UCS_CONFIG_TYPE_UINT},
                                ...
                                unsigned *num_tl_devices_p)
{
    ...
    unsigned i, count, idx;

    if ((tl_devices_p == NULL) || (num_tl_devices_p == NULL)) {
        return UCS_ERR_INVALID_PARAM;
    }

    count = 0;
    for (i = 0; i < obmm_md->topology.num_devices; ++i) {
        if ((obmm_md->topology.devices[i].flags & UCT_OBMM_DEVICE_FLAG_DEGRADED) ||
            (obmm_md->topology.devices[i].primary_ctl == NULL) ||
            (obmm_md->topology.devices[i].res_domain == NULL)) {
            continue;
        }
        ++count;
    }
    ...
    devices = ucs_calloc(count, sizeof(*devices), "obmm_tl_devices");
    if (devices == NULL) {
    ...
    for (i = 0; i < obmm_md->topology.num_devices; ++i) {
        ...
        ucs_strncpy_safe(devices[idx].name, obmm_md->topology.devices[i].name,
                         sizeof(devices[idx].name));
        devices[idx].type       = UCT_DEVICE_TYPE_SHM;
        devices[idx].sys_device = obmm_md->topology.devices[i].caps.sys_device;
        ++idx;
    }

    *tl_devices_p     = devices;
    *num_tl_devices_p = count;
    return UCS_OK;
}
                                         ...
                                         uct_iface_attr_t *attr)
{
    ...
    attr->latency                = iface->latency;
    attr->bandwidth.dedicated    = iface->super.config.bandwidth;
    attr->bandwidth.shared       = iface->device->caps.shared_bw;
    attr->overhead               = iface->overhead;
    attr->priority               = iface->priority;
                                               ...
                                               uct_iface_addr_t *addr)
{
    ...
    uct_obmm_iface_addr_t *iface_addr = (uct_obmm_iface_addr_t*)addr;

    iface_addr->version      = UCT_OBMM_IFACE_ADDR_VERSION;
    iface_addr->device_index = iface->device->index;
    iface_addr->md_generation = iface->md_generation;
    iface_addr->iface_uuid   = iface->iface_uuid;
    return UCS_OK;
}
                               ...
                               const uct_iface_is_reachable_params_t *params)
{
    ...
    if (addr->version != UCT_OBMM_IFACE_ADDR_VERSION) {
        uct_iface_fill_info_str_buf(params, "iface address version mismatch");
        return 0;
    }

    if (addr->md_generation != iface->md_generation) {
        uct_iface_fill_info_str_buf(params, "md generation mismatch");
        return 0;
    }

    if (addr->device_index != iface->device->index) {
        uct_iface_fill_info_str_buf(params, "socket device mismatch");
        return 0;
    }

    if (!uct_sm_iface_is_reachable(tl_iface, params)) {
        uct_iface_fill_info_str_buf(params, "ipc namespace is not reachable");
        return 0;
    }
...
{
    uct_obmm_md_t *obmm_md               = ucs_derived_of(md, uct_obmm_md_t);
    uct_obmm_iface_config_t *obmm_config = ucs_derived_of(tl_config,
    ...
    status = uct_obmm_device_catalog_lookup(obmm_md, params->mode.device.dev_name,
                                            &self->device);
    if (status != UCS_OK) {
        ucs_error("failed to resolve obmm device '%s': no controller bound",
                  params->mode.device.dev_name);
        return status;
    }

    self->iface_uuid    = ucs_generate_uuid((uintptr_t)self);
    self->md_generation = obmm_md->generation;
    self->latency       = ucs_linear_func_make(obmm_config->latency, 0);
    self->overhead      = obmm_config->overhead;
    self->priority      = obmm_config->priority;
    return UCS_OK;
}
