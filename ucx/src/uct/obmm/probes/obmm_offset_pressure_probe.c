...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "Options:\n"
            "  --memids LIST             comma/space separated memids\n"
    ...
    "  --fence-each              add a bus fence after each store/load\n"
            ...
            "Default memids are 71..140 unless --memids or --memid-file is "
            "provided.\n",
            prog);
}
...
static int parse_memid_tokens(memid_list_t *list, char *text)
{
    ...
    for (tok = strtok_r(text, delim, &saveptr); tok != NULL;
        ...
        if ((parse_u64(tok, &memid) != 0) || (append_memid(list, memid) != 0)) {
            return -1;
        }
    }
...
static uint64_t access_offset(const probe_opts_t *opts, size_t memid_index,
                              size_t *color_count_p, size_t *color_index_p)
{
    ...
    if (opts->layout == LAYOUT_SAME) {
        ...
        color_count = opts->memids.count;
    }

    color_index = (color_count == 0) ? 0 : (memid_index % color_count);
...
int main(int argc, char **argv)
{
    ...
    offset = access_offset(&opts, memid_index, &color_count, &color_index);

    access_size = (opts.mode == MODE_HANDOFF) ?
                  (sizeof(handoff_record_t) + opts.bytes) :
                  sizeof(uint64_t);

    if (open_one_map(memid, offset, access_size, &map) != 0) {
        return 1;
    }
    ...
    printf("OBMM_OFFSET_PRESSURE rank=%d size=%d local_rank=%d local_size=%d "
           "mode=%s role=%s layout=%s colors=%zu color_index=%zu "
           "memid=%" PRIu64 " memid_index=%zu offset=%" PRIu64
           ...
           (role == 1) ? "writer" : ((role == 2) ? "reader" : "single"),
           layout_name(opts.layout), color_count, color_index, memid,
           memid_index, offset, offset & (OBMM_2M - 1u), opts.color_step,
           opts.bytes, opts.fence_each, ops, elapsed, ops / elapsed,
           (ops == 0) ? 0.0 : (elapsed * 1e9 / (double)ops), aux, checksum,
           status ? "FAIL" : "OK");
