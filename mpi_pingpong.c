/*
 * mpi_pingpong.c -- 2-rank ping-pong latency.
 *
 * Sweeps small sizes that fit obmm am_short. Reports half round-trip
 * (one-way) average latency in microseconds for each size.
 *
 * Sizes are intentionally <= 1900 bytes to stay under our am_short cap
 * (~ fifo_elem_size minus headers).
 */
#include <mpi.h>
#include <stdio.h>
#include <stdlib.h>
...
static const int sizes[] = {1, 8, 64, 256, 1024, 1900};
...
int main(int argc, char **argv) {
    ...
    if (size != 2) {
        if (rank == 0) {
            fprintf(stderr, "mpi_pingpong needs exactly 2 ranks (got %d)\n",
                    size);
        }
        MPI_Finalize();
        return 1;
    ...
    for (i = 0; i < nsizes; i++) {
        ...
        if (rank == 0) {
            double lat_us = (t1 - t0) * 1e6 / (2.0 * iters);
